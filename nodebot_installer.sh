...
while true; do
    ...
    if [[ "$USER" == "root" ]]; then
        echo -e "${ERROR}错误: 'root' 不能作为用户名。请选择一个不同的用户名。${NC}"
    elif [[ "$USER" =~ [^a-zA-Z0-9] ]]; then
        echo -e "${ERROR}错误: 用户名包含禁止的字符。只能使用字母和数字字符。${NC}"
    else
        break
    fi
done
...
while true; do
    ...
    if [[ "$PASSWORD" =~ [^a-zA-Z0-9] ]]; then
        echo -e "${ERROR}错误: 密码包含禁止的字符。只能使用字母和数字字符。${NC}"
    else
        break
    fi
done
...
echo -e "${SUCCESS}XRDP 配置已更新为使用较低的分辨率 (1280x720)。${NC}"
...
# 最终提示信息
echo -e "${SUCCESS}安装完成。已安装 XFCE 桌面、XRDP、远程桌面工具以及桌面快捷方式。${NC}"
